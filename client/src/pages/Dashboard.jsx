...
const CompanyCard = ({ title, color, cId }) => {
  ...
  return (
    <div className="border rounded p-2 m-2 outlineSwapGreen"
      style={{ width: '300px', borderColor: `#${color ? color : 'ffffff'}` }}
    ...
    <>
      ...
      <div>
        {companies.map((company, i) => {
          return (
            <CompanyCard cId={company.id} title={company.name} color={company.color} key={`CompanyCard-${i}`}/>
          )
        })}
      </div>
    </>
